...
cdef double * calc_coefficients(double h2, double k2, int n, int p):
    cdef double *eigv
    cdef void *bufferp
    _global_bufferp = bufferp
    eigv = lame_coefficients(h2, k2, n, p, &bufferp, 1, 1)
    return eigv
...
def _ellipsoid(double h2, double k2, int n, int p, double s):
    ...
    global _global_p
    global _global_eval
    global _global_bufferp

    eval = calc_coefficients(h2, k2, n, p)
